...
def xla_shape_to_result_shape(xla_shape):
  ...
  # JaxType, i.e. that the mapping is bijective. That assumption could be
  # relaxed, but it would mean we need to do a bit more bookkeping on the Python
  # side to track abstract values of outputs.
  if xla_shape.is_tuple():
